...
## Sleeve B — SHORT mode: post-spike exhaustion fade (`mr_spike_fade`)

**Added 2026-08-13. ⚠ PROPOSED — not yet live.** Written and pre-registered
here first, same discipline as every other change in this file. **Does
not trade until a human adds it to `gates.md`'s trigger set by hand** —
see the activation checklist at the end of this section. The agent must
not treat this trigger as active just because it is described here.

**What this is for:** a stock that suddenly spikes — gap up, often on an
earnings or news catalyst — and looks over-extended enough that the pop
itself is due to give some of it back over the next few days. This is the
short-horizon mirror of the RSI(2) bounce sleeve above, and it is
expressed **only through long puts** (equity shorting is forbidden
outright, per `gates.md`).

**Read `framework.md` item 6 before touching this section.** This sleeve
sits directly next to PEAD (already Tier A evidence in this system,
`framework.md` item 5), which says the *average* post-earnings-beat stock
drifts **up** for weeks. Naive gap-fading is well documented as a losing
strategy. This trigger is deliberately narrower than "stock jumped, short
it" — it only fires once the tape has **already started** reversing, not
in anticipation of a reversal. That is the load-bearing design choice; see
below.

### Regime

**Not a special case — it inherits the existing counter-trend machinery
mechanically.** A bearish setup while SPY > SMA(200) (the common case)
is classified `counter_trend=true` by the same rule already in
`gates.md`, and faces the same five counter-trend gates (1 position max
across the whole book, half risk, ADX(14) > 30 on a direct call, RS bar
≤0.25, no degraded news data) as any Sleeve A short trigger. No new
regime logic — this reuses what exists.

**⚠ One real friction, flagged rather than papered over:** the ADX > 30
counter-trend floor was built for Sleeve A's `breakdown_7d`, where an
established downtrend is the whole premise. `mr_spike_fade`'s premise is
the *opposite* — a stock that has been strong (often still above its own
uptrend's averages) that just cracked. Requiring ADX > 30 on it may be
close to self-contradictory, the same failure mode that starved the
`pullback` trigger in Sleeve A. **This needs a human decision at
activation**, not an assumption: either (a) apply the floor anyway and
accept this sleeve may rarely fire, or (b) exempt it the same way this
sleeve's LONG side is already exempted from the 3%-stop ceiling, with the
reasoning recorded in `gates.md` at the time. Left unresolved here on
purpose.

### Tier 0-MR-short — candidate discovery, zero new calls

Two free sources, both already computed by other routines:

1. **`premarket_notes.md`** — the premarket routine's whole job is
   flagging notable overnight movers with an identified catalyst. Any
   large gap-up with a catalyst noted there is a candidate the moment the
   morning run starts. This is the primary source.
2. **The shared Tier 0 scan** (same universe as Sleeve A/B-long) — take
   the top 10 by the scan's own `RSI(14)` column, **highest** this time
   (mirrors Tier 0-MR's "lowest 10," inverted). Free, no new scan.

### Tier 1-MR-short — the spike/extension template

All required:
- A single-session return (last completed session vs. the one before)
  **≥ 8%**, OR a 3-session cumulative return **≥ 15%** — the "sudden
  jump" itself. Basis: fade-worthy gaps are described in trading-education
  sources as concentrated above an ~8% gap size; below that, ordinary
  daily noise dominates and there's nothing distinct to fade.
- **Relative volume ≥ 1.4x** on the spike session (O'Neil confirmation
  threshold, reused, computed the same way as the breakout sleeve — never
  the scan's `session="all"` column). This is a **reality filter on the
  spike, not a fade signal** — it just confirms the move was real, not
  noise. Do not read it as "fade because volume was high"; the research
  says the opposite (higher-volume gaps hold up better on average). The
  actual fade timing comes from Tier 2.
- **Catalyst confirmed** within the last 3 calendar days — an earnings
  report (`get_earnings_calendar`/`get_earnings_results`) or a named
  catalyst in `premarket_notes.md`. No catalyst, no trade: this keeps
  `mr_spike_fade` distinct from Sleeve A's `momentum_vol`, which is
  catalyst-agnostic.

### Tier 2-MR-short — trigger: `mr_spike_fade`

Both conditions, on the last completed daily bar plus the live quote —
structurally the exact mirror of `mr_reversal`'s two conditions above:
- **RSI(2) ≥ 90** on the last completed session (ConnorsRSI overbought
  convention — stricter than mirroring 15 as 85; 90 is what the
  practitioner literature actually uses for the overbought side), and
- **Current price < that session's close** — the first down-print.

**This second condition is the whole point, not a detail.** The system
does not predict the reversal; it waits for the market to already be
showing one, exactly as the long side waits for the first up-close rather
than buying a knife mid-fall. This is the specific mechanism that keeps
the trigger from contradicting PEAD (see `framework.md` item 6) — it
only fires on the subset of spikes that have already cracked, not on the
average spike, which PEAD says keeps drifting.

**No sub-daily/intraday version of this exists here on purpose.** This
system runs three times a day at fixed times and holds 3-10 days —
`framework.md`'s opening line is explicit that "nothing here is designed
for sub-daily timeframes." A same-day "lost VWAP by 11am" fade (which
some retail gap-fade material describes) is a day-trading rule and does
not fit this system's data or cadence. Don't add one.

### Tier 3-MR-short — news gate and logged (not gating) context

- Same standard as counter-trend: `NEWS_SENTIMENT` must not be
  `UNAVAILABLE`. If the Alpha Vantage quota is exhausted, take no
  `mr_spike_fade` trade that run.
- **Do not gate on the sentiment score's direction yet.** Log it. A
  directional threshold here (e.g. "only fade if sentiment is merely
  neutral, not strongly positive") is intuitively appealing but currently
  unevidenced on this system's own data — the same reasoning
  `tool_verification.md` already applies to Stocktwits `bullish_pct`.
  Earn the gate on a closed-trade record, don't assume it.
- **Log, don't gate:** 52-week range position and ATR-distance from
  SMA20 (`(price - sma20) / atr14`), the exhaustion-gap/blow-off
  framing from `framework.md` item 6. Both are cheap (already-fetched
  fields, no new calls) and Tier C evidence — record them in
  `trade_journal.md`'s narrative and `trade_log.csv`'s `notes` field
  until there's a track record to justify a real gate or a new column.

### Sizing and instrument

Identical arithmetic to the LONG side of this sleeve, mirrored:
- Same 1.5x ATR14 stop, same exemption from `gates.md`'s 3% ceiling and
  for the same reason — this setup selects elevated-ATR names by
  construction (a stock only prints `RSI(2) ≥ 90` by just having moved
  violently).
- Risk budget: **0.2% of NAV** (counter-trend rate), since this will
  classify as counter-trend on nearly every occurrence while SPY is
  bullish, which is the normal regime.
- **Puts only — no equity expression exists.** SHORT mode has no choice
  to make here any more than it does elsewhere in this system: shorting
  stock is forbidden, so if no put or put debit spread fits within the
  $300 premium cap and the standard options gates (30+ DTE, delta
  0.40-0.60 absolute, OI ≥500, spread ≤10%), **take no trade** rather than
  loosen anything. Same options-overlay mechanics as the rest of the
  system: underlying stop/R-target computed at entry and stored in
  `positions.md`, never recomputed later.

### Exits

| Rule | Condition |
|---|---|
| `stop` | Underlying closes back above entry + 1.5x ATR14 |
| `mr_target` | **RSI(2) < 30** — the drop has played out (mirror of the long side's `> 70`) |
| `trend_break` | Underlying reclaims and closes back above its pre-spike level (the level it gapped from) — the thesis is dead |
| `dte_21` / `dte_50pct` | Standard options checkpoints, same as every other options position |
| `time_stop` | **5 trading days** — same as the long side; if the fade hasn't happened in a week, it isn't going to |

Precedence: `stop` → `mr_target` → `trend_break` → `time_stop`.

### Pre-registered review criterion

Fixed now, before any trade has closed, same discipline as `momentum_vol`
and the RSI(2) threshold change:

> Once **≥ 10 `mr_spike_fade` trades have closed**, compute mean R and win
> rate, sliced separately from every other trigger (never pooled with the
> long side of this sleeve, and never with Sleeve A shorts). If mean R is
> negative, retire the trigger rather than re-tuning its thresholds on
> the same sample that produced the negative result. **Fewer than 10
> closes is not a result.**

### Activation checklist (human, hand-edit `gates.md`)

This section is inert until all of the following are added to `gates.md`
by hand, per its own rule that only a human may edit it:

1. Add `mr_spike_fade` as a recognized SHORT trigger for the
   mean-reversion sleeve (mirroring how `mr_reversal`'s LONG entry is
   documented there today).
2. Decide and record the ADX > 30 counter-trend friction noted above —
   apply it as-is, or exempt this trigger with a stated reason.
3. Decide whether `mr_spike_fade` shares the existing **max 2
   mean-reversion positions** cap with the LONG bounce side, or gets its
   own — recommendation: **share it**, same underlying strategy family,
   same 6-slot book.
4. Confirm the **max 1 counter-trend position** cap is understood to
   apply here too (it will, mechanically, via the existing `counter_trend`
   classification — no new rule needed, just noting it isn't a separate
   budget).
...
- **2026-08-13 — `mr_spike_fade` proposed (Sleeve B, SHORT mode, via puts
  only), by user request.** Fades a stock that spiked (typically on
  earnings/news) once the tape shows the pop already cracking —
  `RSI(2) ≥ 90` plus a first down-close, mirroring the long side's
  `RSI(2) < 15` plus first up-close. Deliberately narrower than "short
  every jump": that would directly contradict PEAD, already Tier A
  evidence in `framework.md` item 5, and multiple sources confirm naive
  earnings-gap fading is a documented losing strategy. See
  `framework.md` item 6 for the full reconciliation and evidence tiering,
  and this file's "Sleeve B — SHORT mode" section for the funnel.
  **⚠ Not active** — pre-registered here first; requires a human
  hand-edit to `gates.md` per the activation checklist before it can
  fire. Zero trades yet, nothing to evaluate.
