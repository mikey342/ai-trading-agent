...
| **`momentum_vol` experiment** | **Running from 2026-08-12.** `breakout_52w` keeps its 2% window; a new `momentum_vol` trigger admits the population it excluded (EMA direction + volume, no level test) under a separate label | User decision. Removing the 2% gate outright would have made `breakout_52w` a superset of `breakout_20d` and, via the most-specific-label rule, erased `breakout_20d` from the log — destroying the comparison it exists to enable. Admitting the population separately answers the same question while keeping every trigger measurable. **Pre-registered review: at ≥10 closed `momentum_vol` trades, retire it if its mean R trails `breakout_52w` by ≥0.3R. Fewer than 10 closes is not a result.** |
| **Breakout/breakdown volume confirmation** | `relative_volume ≥ 1.4` — last **completed** session's volume ÷ 30-day average, **computed** from `get_equity_fundamentals`, never read from the scan's `session="all"` column | Required by all four level-based triggers — `breakout_52w`, `breakout_20d`, `breakdown_52w`, `breakdown_20d`. (`breakdown_52w` was missing this requirement until 2026-08-11 — an asymmetry that would have let bearish 52-week-low entries skip the participation check every long must pass. Closed on symmetry grounds; the SHORT mirror is documented as mirroring all three triggers.) Basis: O'Neil's 40%-above-average rule, supported by O'Neil Global Advisors' 1995-2021 study finding volume-confirmed breakouts significantly outperform. Replaced an invented EMA-spread momentum test on 2026-08-11 — see `strategy.md`. **Never use a partial intraday volume**: it would understate every name and reject everything. |
