...
| **Risk level** | Worst of: SPY vs SMA(50), and VIX. **NORMAL** (SPY > 50DMA and VIX < 20) → full rules. **CAUTION** (SPY < 50DMA or VIX 20-25) → max 1 new trade/run, no new index-sleeve entries, counter-trend disabled. **STRESSED** (VIX ≥ 25) → no new entries at all. | Gates *new* commitment only — existing positions are reviewed and exited identically at every level. Added 2026-08-11 to cover the 200DMA's lag: SPY can fall 10-15% before crossing it. In March 2026 VIX crossed 25 roughly three weeks before SPY bottomed. Thresholds are conventions fitted to one year of observed VIX distribution, not backtested — revisit if VIX's baseline shifts. |
