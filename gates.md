...
| **Mean-reversion entry** | `RSI(2) < 10` on the last completed session, **and** price > SMA(200), **and** current price above that session's close | Connors & Alvarez (2008). The SMA(200) condition is load-bearing — removing it inverts the edge into the falling-knife trade. The up-close requirement is a deliberate deviation from pure Connors, trading part of the bounce for not catching a knife mid-fall. |
