...
| **Breakout volume confirmation** | `relative_volume ≥ 1.4` — last **completed** session's volume ÷ 30-day average, **computed** from `get_equity_fundamentals`, never read from the scan's `session="all"` column | Required by both breakout triggers. Basis: O'Neil's 40%-above-average rule, supported by O'Neil Global Advisors' 1995-2021 study finding volume-confirmed breakouts significantly outperform. Replaced an invented EMA-spread momentum test on 2026-08-11 — see `strategy.md`. **Never use a partial intraday volume**: it would understate every name and reject everything. |
