...
| `trigger` | open/reject | Which entry rule fired. LONG: `breakout_7d`, `momentum_vol`, `pullback`. SHORT: `breakdown_7d`, `momentum_vol`, `rally_to_resistance`. MEAN-REVERSION: `mr_reversal`. **Record the most specific trigger that fires**: `breakout_7d` → `momentum_vol` — a row is labelled `momentum_vol` only when no level test passed, which is what makes that experiment measurable. **Slice performance by this column.** The pre-registered comparison is `momentum_vol` vs `breakout_7d` at ≥10 closed `momentum_vol` trades; fewer than 10 closes is not a result. **Historical note:** rows dated 2026-08-11 carry `breakout_52w`, a trigger removed on 2026-08-12 along with `breakdown_52w`. Those labels were accurate when written — keep them, but do not pool them into the current comparison. |
