...
| `trigger` | open/reject | Which entry rule fired. LONG: `breakout_52w`, `breakout_20d`, `pullback`. SHORT: `breakdown_52w`, `breakdown_20d`, `rally_to_resistance`. `momentum_vol` (EMA direction + volume, **no level test** — the experimental population admitted 2026-08-12 when the 2% proximity gate was relaxed). MEAN-REVERSION: `mr_reversal`. **Record the most specific trigger that fires**: `breakout_52w` → `breakout_20d` → `momentum_vol`, so that `momentum_vol` isolates exactly the trades the old gate would have blocked. When both breakout triggers qualify, record the stronger one (`breakout_52w`). **Slice performance by this column** — `breakout_20d` is the weaker level signal and was added to fill a coverage gap; if it underperforms the other two once trades close, it should be removed rather than kept on the argument that introduced it. |
